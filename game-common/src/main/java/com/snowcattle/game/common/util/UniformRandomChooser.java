...
/**
 * 一个均匀分布的随机数序列，一般用于游戏中物品的掉落 例如 A 2% B 3% c %6 ...
 *
 * 用法 初始化阶段 UniformRandomChooser<Item> urc = new UniformRandomChooser<Item>; urc.addObject(0.1,
 * item1); urc.addObject(0.2, item2); urc.addObject(0.3, item3); urc.addObject(0.4, item4);
 * urc.compile();
 *
 * 使用阶段 urc.getObject(0.576);(返回item3)
 *
 * @author jackflit
 */
public class UniformRandomChooser<T> {
	...
	/**
	 * 根据概率返回物品 rate如果在[0, 1.0]之外会按照边界值处理 一般来说，只要物品列表不为空，就不会返回空
	 */
