...
//		int percentageOfPrecision = GlobalManager.getGameConstants().getPercentageOfPrecision();
	...
	/**
	 * 根据自己的位置数[selfPos]，按照间隔[interval]，取指定数量[resultMax]个位置数 注：从自己开始，即含自身
	 *
	...
	/**
	 * 取列表中某一对象附近的几个对象 注：从左右两边各取nearByNum个+自身。如果其中一头不足，则从另一头补足，不考虑两头都不足的情况！ 如果自己没在列表中，则默认为列表的最后一个
	 *
